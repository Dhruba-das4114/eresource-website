...
$sql_subjects = "SELECT * FROM subjects WHERE id = 41"; 
                    ...
                    <td>
                        ...
                        if ($result_papers->num_rows > 0) {
                            while ($paper = $result_papers->fetch_assoc()) {
                                $file_name = htmlspecialchars($paper["file_name"]);
                                $paper_path = "uploads/" . $file_name;
                                $full_path = __DIR__ . "/" . $paper_path;

                                if (file_exists($full_path)) {
                                    echo '<a href="' . $paper_path . '" target="_blank" download>' . htmlspecialchars($paper["year"]) . '</a><br>';
                                } else {
                                    echo "❌ File Not Found for " . htmlspecialchars($paper["year"]) . "<br>";
                                }
                            }
                        } else {
